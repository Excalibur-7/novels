async function fetchNovelContent(url) {
	...
	try {
		...
		// ✅ CAPTCHA 조기 감지
		const isCaptcha = html.includes("자동화된 접근") || html.includes("보안문자") || html.includes("보안 확인");
		if (isCaptcha) {
			console.warn("[fetchNovelContent] CAPTCHA 페이지 감지됨");

			// ✅ 사용자에게 CAPTCHA 해결 요청 및 새 탭 열기
			const goToCaptcha = confirm(`⚠️ CAPTCHA가 감지되었습니다.\n\n${url}\n\n새 탭에서 열고 해결하시겠습니까?`);
			if (goToCaptcha) {
		...
		const titleSelectors = [
			".toon-title", ".view-title", "h1.title", ".post-title", ".entry-title"
		];
		...
		const contentSelectors = [
			"#novel_content", ".novel-content", ".view-content", ".entry-content", ".post-content"
		];
		let content = null;
		...
		if (!content) {
			console.warn("[fetchNovelContent] 콘텐츠 요소 없음");
			return null;
		}
